...
def adjust_learning_rate(optimizer,epoch,lr):
	learning_rate = lr*((0.2**int(epoch>=20))*(0.2**int(epoch>=40))*(0.2**int(epoch>=60))*(0.2**int(epoch>=80)))
...
def main():
	...
	train_loader = DataLoader(
		datasets.CIFAR100(root='/input', train=True, download=True,transform=
						 transforms.Compose([
	...
	test_loader = DataLoader(
		datasets.CIFAR100(root='/input', train=False,
