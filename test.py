...
train_data = torchvision.datasets.CIFAR100(root='/home/zdw/data/CIFAR-100',train=True,download=True,transform=transform_train)
test_data = torchvision.datasets.CIFAR100(root='/home/zdw/data/CIFAR-100',train=False,download=True,transform=transform_test)
...
if args.resume:
        ...
        print("Building net...")
        net = wideResNet.Wide_ResNet28x10()
...
def train(epoch):
        ...
        for batch_id,(inputs,targets) in enumerate(train_loader):
                ...
                train_writer.add_scalar('loss',loss.data.item(),epoch)
                train_writer.add_scalar('train_acc',torch.from_numpy(np.array(correct/total)),epoch)
def test(epoch):
        global best_acc
        net.eval()
        test_loss = 0
        ...
        '''if acc >= best_acc:
                ...
                best_acc = acc'''
        test_writer.add_scalar('loss',loss.data.item(),epoch)
        test_writer.add_scalar('test_acc',torch.from_numpy(np.array(acc)),epoch)
